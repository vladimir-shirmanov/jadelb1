...
import com.company.logic.common.*;
...
public class PLParser extends Parser<Sentence> {
    ...
    private List<ParseNode> groupSimplerSentencesByConnective(Connective connectiveToConstruct, List<ParseNode> parseNodes) {
        ...
        for (int i = parseNodes.size() - 1; i >= 0; i--) {
            ...
            if (parseNode.node instanceof Connective) {
                ...
                if (tokenConnective == Connective.NOT) {
                    // A unary connective
                    if (i + 1 < parseNodes.size() && parseNodes.get(i + 1).node instanceof Sentence) {
                        if (tokenConnective == connectiveToConstruct) {
                            ComplexSentence newSentence = new ComplexSentence(connectiveToConstruct, (Sentence) parseNodes.get(i + 1).node);
                    ...
                    // A Binary connective
                    if ((i - 1 >= 0 && parseNodes.get(i - 1).node instanceof Sentence) && (i + 1 < parseNodes.size() && parseNodes.get(i + 1).node instanceof Sentence)) {
                        // A binary connective
                        if (tokenConnective == connectiveToConstruct) {
                            ComplexSentence newSentence = new ComplexSentence(connectiveToConstruct, (Sentence) parseNodes.get(i - 1).node, (Sentence) parseNodes.get(i + 1).node);
        ...
        if (parseNodes.size() - toSubtract != newParseNodes.size()) {
            throw new ParserException("Unable to construct sentence for connective: " + connectiveToConstruct + " from: " + parseNodes, getTokens(parseNodes));
        }
    ...
    private List<ParseNode> parseLevel(int level) {
        ...
        if (level > 0 && lookAhead(1).getType() == LogicTokenTypes.EOI) {
            throw new ParserException("Parser Error: end of input not expected at level " + level, lookAhead(1));
        }
