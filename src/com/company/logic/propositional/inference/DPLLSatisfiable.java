...
import com.company.logic.propositional.visitors.ConvertToConjunctionOfClauses;
import com.company.logic.propositional.visitors.SymbolCollector;
import com.company.wumpusworld.util.datastructure.Pair;
...
public class DPLLSatisfiable implements DPLL {
    ...
    public boolean dpll(Set<Clause> clauses, List<PropositionSymbol> symbols, Model model) {
        // if every clause in clauses is true in model then return true
        if (everyClauseTrue(clauses, model)) return true;

        // if some clause in clauses is false in model then return false
        if (someClauseFalse(clauses, model) || Tasks.currIsCancelled()) return false;
        ...
        if (pAndValue != null) {
            ...
            return dpll(clauses, minus(symbols, pAndValue.getFirst()), model.union(pAndValue.getFirst(), pAndValue.getSecond()));
        }
        ...
        return dpll(clauses, rest, model.union(p, true)) || dpll(clauses, rest, model.union(p, false));
    }
    ...
    protected List<PropositionSymbol> getPropositionSymbolsInSentence(Sentence s) {
        List<PropositionSymbol> result = new ArrayList<PropositionSymbol>(SymbolCollector.getSymbolsFrom(s));
    ...
    protected Pair<PropositionSymbol, Boolean> findPureSymbol(List<PropositionSymbol> symbols, Set<Clause> clauses, Model model) {
        ...
        if (candidatePurePositiveSymbols.size() > 0) {
            result = new Pair<PropositionSymbol, Boolean>(candidatePurePositiveSymbols.iterator().next(), true);
        } // We have a negative pure symbol
        else if (candidatePureNegativeSymbols.size() > 0) {
            result = new Pair<PropositionSymbol, Boolean>(candidatePureNegativeSymbols.iterator().next(), false);
        }
    ...
    protected List<PropositionSymbol> minus(List<PropositionSymbol> symbols, PropositionSymbol p) {
        List<PropositionSymbol> result = new ArrayList<PropositionSymbol>(symbols.size());
