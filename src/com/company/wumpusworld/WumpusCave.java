...
public class WumpusCave {
    ...
    public WumpusCave(int caveXDimension, int caveYDimension) {
        if (caveXDimension < 1) throw new IllegalArgumentException("Cave must have x dimension >= 1");
        if (caveYDimension < 1) throw new IllegalArgumentException("Case must have y dimension >= 1");
    ...
    public void setPit(Room room, boolean b) {
        ...
        else if (!room.equals(start.getRoom()) && !room.equals(gold)) pits.add(room);
    }
    ...
    public String toString() {
        ...
        for (int y = caveYDimension; y >= 1; y--) {
            for (int x = 1; x <= caveXDimension; x++) {
                Room r = new Room(x, y);
                String txt = "";
                if (r.equals(start.getRoom())) txt += "S";
                if (r.equals(gold)) txt += "G";
                if (r.equals(wumpus)) txt += "W";
                if (isPit(r)) txt += "P";
