package com.company.wumpusworld.search.local;

import com.company.wumpusworld.search.framework.Metrics;
import com.company.wumpusworld.search.framework.problem.GoalTest;
import com.company.wumpusworld.util.Tasks;
import com.company.wumpusworld.util.Util;
...
public class GeneticAlgorithm<A> {
    ...
    // inputs: population, a set of individuals
    // FITNESS-FN, a function that measures the fitness of an individual
    public Individual<A> geneticAlgorithm(Collection<Individual<A>> initPopulation, FitnessFunction<A> fitnessFn, GoalTest<Individual<A>> goalTest, long maxTimeMilliseconds) {
        ...
        do {
            ...
            // until some individual is fit enough, or enough time has elapsed
            if (maxTimeMilliseconds > 0L && (System.currentTimeMillis() - startTime) > maxTimeMilliseconds) break;
            if (Tasks.currIsCancelled()) break;
