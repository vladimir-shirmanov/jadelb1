...
public class GraphSearchReducedFrontier<S, A> extends QueueSearch<S, A> {
    ...
    public Optional<Node<S, A>> findNode(Problem<S, A> problem, Queue<Node<S, A>> frontier) {
        // initialize the explored set to be empty
        if (frontier instanceof PriorityQueue<?>) nodeComparator = ((PriorityQueue<Node<S, A>>) frontier).comparator();
        explored.clear();
    ...
    protected void addToFrontier(Node<S, A> node) {
        if (!explored.contains(node.getState())) {
            ...
            if (frontierNode == null) {
                ...
                if (frontier.remove(frontierNode)) frontierNodeLookup.remove(frontierNode.getState());
                frontier.add(node);
                frontierNodeLookup.put(node.getState(), node);
            }
        }
