package com.company.wumpusworld.search.csp;

import com.company.wumpusworld.search.csp.inference.AC3Strategy;
import com.company.wumpusworld.search.csp.inference.InferenceLog;
import com.company.wumpusworld.search.csp.inference.InferenceStrategy;
...
public class FlexibleBacktrackingSolver<VAR extends Variable, VAL> extends AbstractBacktrackingSolver<VAR, VAL> {
    ...
    public Optional<Assignment<VAR, VAL>> solve(CSP<VAR, VAL> csp) {
        if (inferenceStrategy != null) {
            ...
            if (!log.isEmpty()) {
                ...
                if (log.inconsistencyFound()) return Optional.empty();
            }
        }
    ...
    protected VAR selectUnassignedVariable(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment) {
        ...
        if (varSelectionStrategy != null) vars = varSelectionStrategy.apply(csp, vars);
        return vars.get(0);
    }
    ...
    protected Iterable<VAL> orderDomainValues(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment, VAR var) {
        if (valSelectionStrategy != null) return valSelectionStrategy.apply(csp, assignment, var);
        else return csp.getDomain(var);
    }
    ...
    protected InferenceLog<VAR, VAL> inference(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment, VAR var) {
        if (inferenceStrategy != null) return inferenceStrategy.apply(csp, assignment, var);
        else return InferenceLog.emptyLog();
    }
}
