...
public class CspHeuristics {
    ...
    public static <VAR extends Variable, VAL> VariableSelection<VAR, VAL> mrv() {
        return new MrvHeuristic<>();
    }

    public static <VAR extends Variable, VAL> VariableSelection<VAR, VAL> deg() {
    ...
    public static <VAR extends Variable, VAL> ValueSelection<VAR, VAL> lcv() {
    ...
    public static class LcvHeuristic<VAR extends Variable, VAL> implements ValueSelection<VAR, VAL> {
        ...
        public List<VAL> apply(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment, VAR var) {
            ...
            return pairs.stream().sorted(Comparator.comparing(Pair::getSecond)).map(Pair::getFirst).collect(Collectors.toList());
        }
        ...
        private int countLostValues(CSP<VAR, VAL> csp, Assignment<VAR, VAL> assignment, VAR var, VAL value) {
            ...
            for (Constraint<VAR, VAL> constraint : csp.getConstraints(var)) {
                if (constraint.getScope().size() == 2) {
                    ...
                    if (!assignment.contains(neighbor)) for (VAL nValue : csp.getDomain(neighbor)) {
